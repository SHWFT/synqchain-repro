...
export default function ApprovalsPage() {
  ...
  const pendingApprovals = getMyApprovals(user?.name || user?.email || "Demo User");
  ...
  const handleApprovalAction = async (data: ApprovalActionData) => {
    ...
    try {
      ...
      const updatedPO = {
        ...selectedPO,
        status: actionType === "approve" ? ("approved" as const) : ("cancelled" as const),
  ...
  const openApprovalDialog = (po: PurchaseOrder, action: "approve" | "reject") => {
    setSelectedPO(po);
  ...
  const totalPendingValue = pendingApprovals.reduce((sum, po) => sum + po.total, 0);
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <div className="text-2xl font-bold">
                {pendingApprovals.filter(po => 
                  po.needByDate && new Date(po.needByDate) <= new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
                ).length}
              </div>
